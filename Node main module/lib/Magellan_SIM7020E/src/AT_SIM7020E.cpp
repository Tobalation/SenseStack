...
int AT_SIM7020E::sendJSONPOST( String endpoint,String directory,int protocol, String header,String contentType, String data){
  ...
  while(1){
    if(_Serial->available()){
      data_input=_Serial->readStringUntil('\n');
      Serial.println(data_input);                               
      if(data_input.indexOf(F("OK"))!=-1 || data_input.indexOf(F("ERROR"))!=-1){
        break;
      }
  ...
  if(status){
    ...
    while(1){
      if(_Serial->available()){
        data_input=_Serial->readStringUntil('\n');
        Serial.println("Send 3 :" + data_input);                               
        if(data_input.indexOf(F("OK"))!=-1){
          break;
        }
        else if(data_input.indexOf(F("ERROR"))!=-1) {
    ...
    //if(debug) Serial.println("Create socket success");
     ...
     while(1){
      if(_Serial->available()){
        data_input=_Serial->readStringUntil('\n');
        Serial.println("Receive :" + data_input);    
        if(data_input.indexOf("+CHTTPNMIH:")!=-1) {
          for (int i = 0 ; i < data_input.length(); i++){
            if (data_input[i] == ',' ){
              i++;
              while (data_input[i] != ','){
                responseCode += data_input[i++];
              }
              break;
            }
          }
